...
const createMachineDirectoryBodySchema = z.object({
...
export function createMachinesRoutes(getSyncEngine: () => SyncEngine | null): Hono<WebAppEnv> {
    ...
    app.get('/machines/:id/directory', async (c) => {
        const engine = getSyncEngine()
        if (!engine) {
            return c.json({ success: false, error: 'Machine directory browse unavailable' }, 503)
        }
    ...
    app.post('/machines/:id/directory', async (c) => {
        const engine = getSyncEngine()
        if (!engine) {
            return c.json({ success: false, error: 'Machine directory creation unavailable' }, 503)
        }
        ...
        const body = await c.req.json().catch(() => null)
        const parsed = createMachineDirectoryBodySchema.safeParse(body)
        if (!parsed.success) {
            return c.json({ error: 'Invalid body' }, 400)
        }
