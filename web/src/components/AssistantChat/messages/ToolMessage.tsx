        ...
        <div className="flex flex-col gap-3">
            {props.blocks.map((block) => {
                if (block.kind === 'user-text') {
                    const userBubbleClass = 'w-fit max-w-[92%] ml-auto rounded-xl bg-[var(--app-secondary-bg)] px-3 py-2 text-[15px] text-[var(--app-fg)] shadow-sm'
                ...
                if (block.kind === 'tool-call') {
                    ...
                    return (
                        <div key={`tool:${block.id}`} className="py-1 opacity-60 animate-fade-in-up">
                            <ToolCard
                                api={ctx.api}
...
export function HappyToolMessage({ block }: { block: ToolCallBlock }) {
    ...
    return (
        <div className="py-1 min-w-0 max-w-full overflow-x-hidden opacity-60">
            <ToolCard
                api={ctx.api}
                sessionId={ctx.sessionId}
                metadata={ctx.metadata}
