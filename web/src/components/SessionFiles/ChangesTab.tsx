        ...
        <div className="flex flex-col flex-1 overflow-hidden">
                    ...
                    {stagedExpanded && staged.length > 0 && (
                        <div className="flex items-center gap-2">
                            <button type="button" onClick={toggleAllStaged} className="text-[10px] text-[var(--app-link)] hover:underline">
                                {selectedStaged.size === staged.length ? 'Deselect All' : 'Select All'}
                            </button>
                            <button
                                type="button"
                    ...
                    {unstagedExpanded && unstaged.length > 0 && (
                        <div className="flex items-center gap-2">
                            <button type="button" onClick={toggleAllUnstaged} className="text-[10px] text-[var(--app-link)] hover:underline">
                                {selectedUnstaged.size === unstaged.length ? 'Deselect All' : 'Select All'}
                            </button>
                            <button
                                type="button"
            ...
            <div className="border-t border-[var(--app-divider)] bg-[var(--app-bg)] p-3 flex flex-col gap-2">
                <div className="flex gap-2">
                    <button type="button" onClick={handleStageAll} disabled={unstaged.length === 0 || batchLoading} className="flex-1 text-xs py-1.5 rounded border border-[var(--app-border)] text-[var(--app-fg)] hover:bg-[var(--app-subtle-bg)] disabled:opacity-40">Stage All</button>
                    <button type="button" onClick={handleUnstageAll} disabled={staged.length === 0 || batchLoading} className="flex-1 text-xs py-1.5 rounded border border-[var(--app-border)] text-[var(--app-fg)] hover:bg-[var(--app-subtle-bg)] disabled:opacity-40">Unstage All</button>
                </div>
                <textarea
                    value={commitMessage}
